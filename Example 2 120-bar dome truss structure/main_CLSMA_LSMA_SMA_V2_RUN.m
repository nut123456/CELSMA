...
Times = 10; % Number of independent times you want to run the SMA, LSMA, CLSMA
...
Convergence_curve_CLSMA_all = zeros(Times, T);
best_chaos_index = 1; 
...
for i = 1:Times
    ...
    display(['The optimal fitness of CELSMA (Run ', num2str(i), ') is: ', num2str(Destination_fitness_CLSMA(i))]);
end
...
xlabel('Iteration');
ylabel('Weight (kg)');
semilogy(Convergence_curve_CLSMA(index_CLSMA,:), 'LineWidth', 3);
xlabel('Iterations');
hold on;
semilogy(mean(Convergence_curve_CLSMA_all), '--', 'LineWidth', 3);
xlabel('Iteration');
ylabel('Weight (kg)');
