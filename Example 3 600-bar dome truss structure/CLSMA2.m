...
function [Destination_fitness,bestPositions,Convergence_curve]=CLSMA2(N,Max_iter,lb,ub,dim,fobj,chaos_index)
...
while  it <= Max_iter
    ...
    for i=1:N
        chaos_value = chaos(chaos_index, it, Max_iter, 1); % Generate chaotic value
        for j=1:dim
            if i <= (N/2) 
                weight(SmellIndex(i),j) = 1 + chaos_value * log10((bestFitness - SmellOrder(i))/S + 1); % Use chaos in weight calculation
            else
                weight(SmellIndex(i),j) = 1 - chaos_value * log10((bestFitness - SmellOrder(i))/S + 1);
            end
        end
    end
    ...
    for i=1:N
        if rand<z     %Eq.(24.a)
            ...
            B = randi([1,N]);
            % Use chaotic value for random decision (r1)
            r1 = chaos(chaos_index, it, Max_iter, 1);
            for j=1:dim
                if r1<p    
...
title('\fontsize{12}\bf Computation Time per Iteration (CELSMA)');
